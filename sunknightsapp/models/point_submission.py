...
def pointsupdater(pointsinfo):
    sumPoints = \
        decimal.Decimal(BasicPointSubmission.objects.filter(pointsinfo=pointsinfo,accepted=True,decided=True).aggregate(
            models.Sum('points'))['points__sum'] or 0.0)

    #get points from fights that were lost
    sumPoints+=decimal.Decimal(OneOnOneFightSubmission.objects.filter(pointsinfoloser=pointsinfo,accepted=True,decided=True).aggregate(
...
def updateCurrentPoints(submission):
    ...
    if isinstance(submission, OneOnOneFightSubmission):
        pointsupdater(submission.pointsinfoloser)
